class Monitor {
    ...
    render_log() {
        ...
        this.anomaly_log.forEach(entry => {
            const time = new Date(entry.timestamp * 1000).toLocaleString();
            const delta = (entry.value - entry.emwa).toFixed(2);

            log_table.insertAdjacentHTML('beforeend', `
                <tr>
                    <td>${time}</td>
                    <td>${this.format_metric_name(entry.metric)}</td>
                    <td>${this.format_value(entry.metric, entry.value)}</td>
                    <td>${delta}</td>
                    <td>${entry.z_score.toFixed(2)}</td>
                </tr>
            `);
        });
    }
